...
class TodoistRepository:
    ...
    def get_project_tasks(self, project_name):
        """
        Fetches all active tasks from a specific Todoist project that do not have the "needs_review" tag.
        """
        try:
            tasks_iterator = self.api.filter_tasks(
                query=f"#{project_name} & !@{config.TODOIST_ERROR_TAG}"
            )
            all_tasks = []
    ...
    def add_label_to_task(self, task_id, label_name):
        ...
        try:
            ...
            if label_name not in task.labels:
                ...
                if not is_success:
                    raise Exception(
                        f"Todoist API failed to update task {task_id} with label '{label_name}'."
                    )
